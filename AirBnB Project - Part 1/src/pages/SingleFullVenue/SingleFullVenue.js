...
class SingleFullVenue extends Component {
    ...
    async componentDidMount() {
        ...
        const singleVenue = VenueResponse.data;

        const pointsUrl = `${window.apiHost}/points/get`;
        const pointsResponse = await axios.get(pointsUrl);
        const points = singleVenue.points.split(',')
            .map((point, index) => { return (<Points key={index} pointDesc={pointsResponse.data} point={point} />) })
            ...
            <div className='row single-venue'>
                ...
                <div className='col s8 location-details offset-s2'>
                    <div className='col s8 left-details'>
                        ...
                        <i className="material-icons">done</i>{sv.amenities}</div>
                    </div>
